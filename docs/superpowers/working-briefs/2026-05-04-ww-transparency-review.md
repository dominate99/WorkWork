...
- `dispatch_recommendation`: Stay in a single-orchestrator design pass until the revised written spec is reviewed. Do not launch subagents in this round unless the scope expands beyond interaction design or implementation planning begins. The next revision must close the remaining contract-shape gaps before the spec is approved.
