...
- `relevant_context`:
  ...
  - a staff engineer review of the first implementation plan found two material gaps:
    - the planned edit surface omitted `plugins/workwork/skills/ww-subagent-orchestrator/SKILL.md`, even though repository guidance requires workflow-behavior changes to update the canonical skill contract
    - the verification steps were too generic and did not clearly prove cross-file authority alignment or demonstrate the updated worker packet example
  - the agreed model remains `user constraints -> work_mode -> persona -> goal_tuning`
  - the agreed work-mode authority chain is:
    - packaged `SKILL.md` records the canonical workflow contract
  ...
  - the files that now need aligned edits are:
...
- `constraints`:
  ...
  - keep the new fields and authority rules aligned across all five files
  - do not introduce ambiguous duplicate authority between skill contract, brief, plan, packet, and prompt

## Risk And Structure

- `risk_lenses`:
  - field-name drift across the lower-level files could make the new mode chain decorative instead of executable
...
- `parallelism_assessment`:
  ...
  - one bounded implementation lane is safer than parallel edits because all five files describe one shared runtime chain
- `blocking_dependencies`:
  - the revised design spec and implementation plan must exist before the revised dispatch round is approved
  - the working brief and dispatch plan must be persisted before any packet or real edit work begins
  - the worker prompt wording should be updated only after the packet-contract fields and skill-level rules are settled
- `section_or_workstream_map`:
  - section 1: align the five worker-mode contract surfaces
  - section 2: run a bounded review and verification pass over naming, authority, packet example, and execution-order consistency
...
- `artifact_mappings`:
  ...
  - `artifact_kind`: `doc`
  - `artifact_path`: `docs/superpowers/specs/2026-05-17-worker-work-mode-alignment-design.md`
  - `section_anchors`: `Design`, `Required Contract Changes`
  - `artifact_id`: `worker_mode_impl_plan`
  - `artifact_kind`: `doc`
  - `artifact_path`: `docs/superpowers/plans/2026-05-17-worker-work-mode-alignment.md`
...
- `dispatch_recommendation`:
  - approve one bounded implementation section covering the five aligned worker-mode contract surfaces
  - require a review pass that checks naming alignment, authority alignment, packet-example alignment, and worker execution-order alignment
...
- `review_target_strategy`:
  - review the five modified runtime contract surfaces together
